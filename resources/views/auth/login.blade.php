...
@if(Session::has('success'))
 ...
 @endif
<form action="{{ route('login') }}" method='post' >{{ route('password.update') }}<input type="hidden" name="token" value="{{ $request->route('token') }}">  <div>
...
<div class="field">
                  <label>
                    <span>Username</span>
     <input type="text" name="email" value="{{ old('email') }}" class="@error('email') is-invalid @enderror form-control" class=inpts size=30 autofocus="autofocus">
                  </label>
                   @error('email')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
